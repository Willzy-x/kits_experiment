...
class VNet(nn.Module):
    ...
    def __init__(self, nclass=2 ,elu=True, nll=False, dropout=True, attention='DcpA'):
        ...
        self.down_tr128 = DownTransition(64, 3, elu, dropout=dropout) # from 3 to 2 # 64
        self.down_tr256 = DownTransition(128, 2, elu, dropout=dropout)
        self.down_tr512 = DownTransition(256, 2, elu, dropout=dropout)
        if attention == 'DbA':
            self.attention = DoubleAttention3d(128, 128, reduced_dim=128)
        elif attention == 'DcpA':
            self.attention = DecoupledAttention3d(512, 512)
        elif attention == 'ResA':
        ...
        self.up_tr512 = UpTransition(512, 512, 2, elu, dropout=dropout)
        self.up_tr256 = UpTransition(512, 256, 2, elu, dropout=dropout)
        self.up_tr128 = UpTransition(256, 128, 2, elu, dropout=dropout) # 128 128
    ...
    def forward(self, x):
        ...
        out512 = self.attention(out512)
        # -----------------------------
        out = self.up_tr512(out512, out256)
        out = self.up_tr256(out, out128)
